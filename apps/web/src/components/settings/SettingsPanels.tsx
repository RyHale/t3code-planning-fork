...
export function useSettingsRestore(onRestored?: () => void) {
  ...
  const restoreDefaults = useCallback(async () => {
    ...
    resetSettings();
    onRestored?.();
  }, [changedSettingLabels, onRestored, resetSettings, resetThemeCustomization, setTheme]);
...
export function GeneralSettingsPanel() {
  ...
  const [isThemeCustomizationOpen, setIsThemeCustomizationOpen] = useState(false);
          ...
          control={
            <div className="flex w-full items-center justify-end gap-1.5 sm:w-auto">
              ...
              <Button
                ...
                aria-label={
                  isThemeCustomizationOpen ? "Hide theme customization" : "Show theme customization"
                }
                aria-expanded={isThemeCustomizationOpen}
                onClick={() => setIsThemeCustomizationOpen((open) => !open)}
              >
                <ChevronDownIcon
                  className={`size-4 transition-transform ${
                    isThemeCustomizationOpen ? "rotate-180" : ""
                  }`}
                />
              </Button>
            </div>
